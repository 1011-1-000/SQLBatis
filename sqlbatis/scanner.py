...
class EntityScanner(Scanner):
    ...
    def __init__(self, directory='.', exclude=[]):
        ...
        super(EntityScanner, self).__init__(directory, exclude, '.py')
    ...
    def scan_entities(self):
        ...
        for file in relative_files:
            _module = '.'.join(file.split(os.sep)[1:])
            _entities = self._get_model_class_in_the_module(_module)
            self.entities.extend(_entities)
